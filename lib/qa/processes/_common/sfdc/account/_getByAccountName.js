 ...
 */
module.exports = (sfConn, name, emailAddress = null, personAccountsEnabled = false) => new Promise(async (resolve, reject) => {
    ...
    // if person accounts are enabled and an email address was supplied, include person accounts in the filter query
    if (emailAddress && personAccountsEnabled) {
        filterQuery.$or.push({ PersonEmail: { $like: '%' + emailAddress + '%' } });
    }
