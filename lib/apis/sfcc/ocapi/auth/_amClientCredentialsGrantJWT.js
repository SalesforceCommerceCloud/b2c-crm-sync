...
module.exports = (environmentDef) => new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        // Create the JWT token using environment properties
        const jwtToken = requestLib.createAMJWT(environmentDef);

        // Execute and process the request; wait for the results
        const responseObj = await requestInstance.post(thisRequestDef.url, jwtToken.jwt.requestBody, thisRequestDef);
