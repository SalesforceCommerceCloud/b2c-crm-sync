 ...
 */
module.exports = (envDef, siteId, clientId, customerAuthToken, customerId, profileUpdate) => new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        let thisRequestDef = requestLib.createOCAPIShopRequestDef(ocapiUrlSuffix, siteId, clientId, customerAuthToken);

        // Define the request body details
        thisRequestDef.body = profileUpdate;

        // Execute and process the request; wait for the results
        const responseObj = await b2cRequestInstance.patch(thisRequestDef.url, thisRequestDef.body, thisRequestDef);
