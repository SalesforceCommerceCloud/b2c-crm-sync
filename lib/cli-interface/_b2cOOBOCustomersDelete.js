...
module.exports = commandProgram => {
    ...
    commandProgram
        ...
        .description('Remove the OOBO user created for each configured storefront -- defaults to the storefronts defined in the .env; can be overridden via the CLI')
        .action(async commandObj => {
            ...
            try {
                ...
                console.log(' -- Attempting to retrieve the OOBO Customer Profiles for each storefront');
                ...
                if (resultObj.outputDisplay.verifySites.success.length > 0) {
                    ...
                    customerVerifyResults = await cliAPI.b2cOOBOCustomersVerify(environmentDef, resultObj.siteResults.success);
                    ...
                    if (customerVerifyResults.length === 0) {
                        ...
                        for (let thisResult of customerVerifyResults) {
                            ...
                            await cliAPI.b2cOOBOCustomerDelete(environmentDef, thisResult.profile);
                            ...
                            await cliAPI.sfOOBOB2CSitesReset(environmentDef, thisResult.profile.customer_list);

                        }

                        // Take the collection of siteResults and the customerResults -- and update the customerId site preference
                        console.log(' -- Removing the OOBO Anonymous Customer customerId values from sitePreferences for these sites');
                        sitePreferenceResults = await cliAPI.b2cOOBOSitePrefsUpdate(environmentDef, resultObj.outputDisplay.authenticate.authToken, resultObj.siteResults.success);
