    ...
    commandProgram
        ...
        .action(async commandObj => {
            ...
            try {
                ...
                console.log(environmentDef);

                // Only output the helper guidance if we can resolve the prefix safely
                if (environmentDef.sfHostName.indexOf('.salesforce.com') !== -1 ||
                    environmentDef.sfHostName.indexOf('lightning.force.com') !== -1) {
                    ...
                    salesforceOrgUrlPrefix = salesforceOrgUrlPrefix.replace('lightning.force.com', '');
                    salesforceOrgUrlPrefix = salesforceOrgUrlPrefix.replace(
                        `.${environmentDef.sfLoginUrl.split('.')[0].toLowerCase()}`, '');
