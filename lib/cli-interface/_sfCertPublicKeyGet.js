...
module.exports = commandProgram => {
    ...
    commandProgram
        ...
        .description('Attempts to parse the publicKey from the downloaded Salesforce self-signed certificate keyStore -- and output its contents to the console.')
        .action(async commandObj => {
            ...
            try {
                ...
                console.log(' -- Attempting to parse the publicKey from the downloaded Salesforce self-signed certificate JKS (Java KeyStore)');
                ...
                if (!fs.existsSync(certPath)) { throw new Error(`Unable to locate the certificates directory: ${certPath}`); }
                ...
                if (certFiles.length === 0) { throw new Error(`Unable to find a java keyStore file (.jks extension): ${certPath}`); }
