 ...
 * @description This function is used to verify that authentication against B2C Commerce's
 * Account Manager via JWT is possible.  Commands are abstracted in this manner to facilitate unit
 * testing of each command separately.
 *
...
module.exports = commandProgram => {
    ...
    commandProgram
        ...
        .description('Attempts to authenticate against the specified B2C Commerce Account Manager ' +
            'environment leveraging JWT -- defaults to the .env; can be overridden via the CLI')
        .action(async commandObj => {
            ...
            try {
                ...
                cliUi.cliCommandBookend(
                    commandObj._name,
                    'start',
                    'Attempting to authenticate against the configured Account Manager instance using JWT',
                    environmentDef);
                ...
                if (resultObj.success === true) {
                    ...
                    cliUi.outputResults(
                        ...
                        undefined,
                        'cliTableConfig.b2cJWTOutput');

                } else {

                    // Output the jwt results and structure
                    cliUi.outputResults(
                        resultObj.outputDisplay,
                        undefined,
                        'cliTableConfig.b2cJWTOutput');

                    // Output any error messages captured
                    cliUi.outputResults(
                        resultObj.errorOutputDisplay,
                        undefined,
                        'cliTableConfig.genericFault');

                    // Throw a hard error and invoke the visual error handler
                    throw new Error('Unable to authenticate against the B2C Commerce Account Manager');
