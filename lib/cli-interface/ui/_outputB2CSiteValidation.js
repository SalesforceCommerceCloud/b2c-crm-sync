 ...
 * @param {Object} [errorObj] Represents any error messaging being carried forward to the CLI
...
module.exports = (siteResults, error, hideEnvironmentDef = false) => {
    ...
    if (siteResults.b2cSiteProperties.validationResult !== true) {
        ...
        siteResults.b2cSiteProperties.value.filter(siteKey => !siteResults.b2cSiteProperties.siteResults[siteKey].validationResult).forEach(siteKey => {
    ...
    if (siteResults.hasOwnProperty('validationSummary')) {
