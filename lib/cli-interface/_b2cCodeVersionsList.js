...
module.exports = commandProgram => {
    ...
    commandProgram
        ...
        .description('Retrieves the full collection of code-versions that exist within the specified ' +
            'B2C Commerce environment -- defaults to the .env; can be overridden via the CLI')
        .action(async commandObj => {
            ...
            try {
                ...
                cliUi.cliCommandBookend(
                    commandObj._name,
                    'start',
                    'Attempting to list the B2C code versions from the configured B2C instance',
                    environmentDef);
                ...
                // Throw an error if no code-version was verified
                if (resultObj.outputDisplay.codeVersionList.length === 0) {
                    throw new Error('No code version found on the target instance.');
                }

                // Render the code versions details
                cliUi.outputResults(
                    resultObj.outputDisplay.codeVersionList,
                    undefined,
