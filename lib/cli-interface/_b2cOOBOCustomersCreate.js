                ...
                if (resultObj.outputDisplay.verifySites.success.length > 0) {
                    ...
                    for (let thisResult of customerResults) {
                        ...
                        if (thisResult.exists === true) {
                            console.log(' -- Verified that the following OOBO Anonymous Customer record exists; did not create a new customerProfile');
                        } else {
                            console.log(' -- Could not verify the existence of an OOBO Anonymous Customer record; Creating a new customerProfile');
                        }
                        ...
                        if (thisResult.unableToVerifySFDC === true) {
                            console.log(' -- WARNING! Unable to verify the existence of the corresponding SFDC B2C Customer Profile'.bold.red);
                            console.log(' -- Please consider deleting via crm-sync:oobo:customers:delete and re-create these relationships'.bold.red);
                        }

                        // Iterate over these customers and update the B2C Site definitions with the OOBO customers
                        await cliAPI.sfOOBOB2CSitesUpdate(environmentDef, thisResult.profile);
