...
module.exports = async environmentDef => {
    ...
    return sObjectAPIs.create(connectionResult.conn, 'AuthProvider', {
        ...
        if (resultObj.success === true) {
            console.log(' -- MANDATORY ACTION -- Please make sure to update the Account Manager Client ID with the following Callback URL: '
                + `https://${environmentDef.sfHostName}${environmentDef.sfHostName.charAt(environmentDef.sfHostName.length - 1) === '/' ? '' : '/'}services/authcallback/${environmentDef.b2cInstanceName}`);
        }
    });
};
