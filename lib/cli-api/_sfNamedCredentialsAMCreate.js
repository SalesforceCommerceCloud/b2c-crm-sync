...
/**
 * @function _sfNamedCredentialsAMCreate
...
module.exports = environmentDef => {
    ...
    const templateSuffix = `template_B2C_AMCredentials.namedCredential${fileExt}`;
    // Create the destination file's filename and suffix; personalized with the B2C Instance Name
    const fileSuffix = `${environmentDef.b2cInstanceName}_B2C_AMCredentials.namedCredential${fileExt}`;
    ...
    return sfTemplateCreator(environmentDef, templateFolder, templateSuffix, fileSuffix, templateFileAsString => {
        ...
        templateFileAsString = templateFileAsString.replace(/{{B2C_INSTANCENAME}}/gm, environmentDef.b2cInstanceName);
