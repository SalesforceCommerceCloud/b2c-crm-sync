...
module.exports = (environmentDef, pathScope, pathElement) => new Promise(async (resolve, reject) => {
    ...
    try {
        output.apiCalls.deploy = await deploymentAPI.deploy(environmentDef, archivePath, output.apiCalls.authenticate.authToken);
    } catch (e) {
    ...
    try {
        output.apiCalls.codeVersionActivate.activationResult = await codeVersions.activate(environmentDef, output.apiCalls.authenticate.authToken, output.apiCalls.deploy);
    ...
    try {
        const getDetailResults = await codeVersions.getDetail(environmentDef, output.apiCalls.authenticate.authToken, output.apiCalls.deploy);
