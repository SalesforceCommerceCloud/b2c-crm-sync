...
module.exports = environmentDef => new Promise(async (resolve, reject) => {
    ...
    try {
        output.apiCalls.codeVersionActivate.activationResult = await codeVersions.activate(environmentDef, output.apiCalls.authenticate.authToken);
    ...
    try {
        const getDetailResults = await codeVersions.getDetail(environmentDef, output.apiCalls.authenticate.authToken);
