 ...
 * @returns {Promise}
 */
module.exports = (environmentDef, customerProfile) => new Promise(async (resolve, reject) => {
    ...
    output.apiCalls.b2cCustomerDelete = await dataAPIs.customerDelete(
        baseRequest, b2cAdminAuthToken, customerProfile.customer_list, customerProfile.customer_no);

    // Was a fault encountered when we attempted to delete the customerProfile?
    if (output.apiCalls.b2cCustomerDelete.status === 204 && customerProfile.hasOwnProperty('c_b2ccrm_contactId')) {
