...
module.exports = environmentDef => new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        siteList.filter(siteId => connectedAppCredentials.siteIds.indexOf(siteId) > -1).forEach(siteId => {
            ...
            serviceCredentialTemplateString = serviceCredentialTemplateString.replace('{{SF_PASSWORD}}', environmentDef.sfPassword);
            serviceCredentialTemplateString = serviceCredentialTemplateString.replace('{{SF_SECURITYTOKEN}}', environmentDef.sfSecurityToken);
            serviceCredentialTemplateString = serviceCredentialTemplateString.replace('{{SF_CONSUMERKEY}}', consumerKey);
            serviceCredentialTemplateString = serviceCredentialTemplateString.replace('{{SF_CONSUMERSECRET}}', consumerSecret);
