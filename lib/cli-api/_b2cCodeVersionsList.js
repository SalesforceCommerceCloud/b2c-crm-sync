...
module.exports = environmentDef => new Promise(async (resolve, reject) => {
    ...
    try {
        const codeVersionsResult = await codeVersions.list(environmentDef, output.apiCalls.authenticate.authToken);
        // Create the roll-up summary for the B2C Commerce code versions
        output.apiCalls.codeVersionList.codeVersions = common.createCodeVersionSummary(codeVersionsResult.data);
