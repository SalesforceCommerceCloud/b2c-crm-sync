 ...
 *
 * @param {Function} [processCallback] Represents the callback to execute once processing is complete
 * @return {Object} Returns the output if a process callback isn't defined
 */
module.exports = () => new Promise((resolve, reject) => fsAPI.setupB2CDeploymentFolders().then(output => resolve(output)).catch(err => reject(err)));
