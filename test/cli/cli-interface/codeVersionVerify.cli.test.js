...
describe('Verify a B2C Commerce Code Version via the CLI', function () {
    ...
    cliCommand = parseCLICommand('npm run crm-sync:b2c:cv:verify -- -om json');

    it('should provide a JSON string representing the validated code-version', async function () {
    ...
    it('returns the properties describing the validated / retrieved code-version', async function () {
        ...
        cmd.execute(
            ...
            .then(function (response) {
                ...
                validProperties = config.util.toObject(config.get('unitTests.cliOutputValidationAttributes.verifyCodeVersion'));
                ...
                for (let arrayIndex = 0; arrayIndex < responseJSON.length; arrayIndex++) {
                    ...
                    thisCodeVersion = responseJSON[arrayIndex];

                    // Validate that the environment definition has the required keys
                    assert.hasAnyKeys(thisCodeVersion, validProperties);
